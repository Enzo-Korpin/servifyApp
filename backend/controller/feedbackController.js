...
export const submitFeedback = async (req, res) => {
  ...
  const safeAbort = async () => {
    try {
      ...
      console.error("Failed to abort transaction:", error.message);
    }
  };
  ...
  try {
    ...
    const serviceRequest = await ServiceRequest.findById(requestId).session(
      session
    );
    ...
    if (serviceRequest.customerId.toString() !== customerId.toString()) {
      ...
      return res
        .status(403)
        .json({ message: "Not allowed to submit feedback for this request" });
    }

    if (serviceRequest.status !== "completed") {
      await safeAbort();
      return res.status(400).json({
        message: "Cannot submit feedback unless request is completed",
      });
    }
    ...
    updatedWorker.ratingAvg =
      Math.round(
        (updatedWorker.ratingSumX2 / updatedWorker.ratingCount / 2) * 100
      ) / 100;
