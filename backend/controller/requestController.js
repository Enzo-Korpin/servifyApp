...
export const createServiceRequest = async (req, res) => {
    try {
        ...
        const { workerId, requestedSkill, message, addressText, location } = req.body;
        ...
        if (!workerId || !requestedSkill) {
            return res.status(400).json({ message: "workerId and requestedSkill are required" });
        }
        if (!location || location.lng == null || location.lat == null) {
            return res.status(400).json({ message: "location.lng and location.lat are required" });
        }
        ...
        if (!workerSkills.includes(requested)) {
            return res.status(400).json({ message: "Worker does not have the requested skill" });
        }
...
export const getServiceRequestsForCustomer = getServiceRequestsByRole("customer", "customerId");
export const getServiceRequestsForWorker = getServiceRequestsByRole("worker", "workerId");
...
export const cancelServiceRequest = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Service request cancelled", data: request });
    } catch (err) {
        return res.status(500).json({ message: "Server error", error: err.message });
    }
};
...
export const acceptServiceRequest = async (req, res) => {
    try {
        ...
        if (request.status !== "pending") {
            return res.status(400).json({ message: `Cannot accept request with status '${request.status}'` });
        }
